...
import numpy as np # ESSENZIALE: Pinocchio si basa su NumPy per tutte le matrici e i vettori.
import os
from ament_index_python.packages import get_package_share_directory
import subprocess
import re
from geometry_msgs.msg import PoseStamped # Importa PoseStamped per pubblicare la posa
# Rimuoviamo: from scipy.spatial.transform import Rotation as R 

class UR10eFKNode(Node):
    """
    Nodo ROS 2 per calcolare la Cinematica Diretta (FK) dell'UR10e 
    utilizzando Pinocchio e pubblicare la posa dell'end-effector 
    come PoseStamped, senza dipendenza da SciPy.
    """
    def __init__(self):
        super().__init__('ur10e_fk_node')

        self.get_logger().info('Initializing UR10e Forward Kinematics Node (Pinocchio Only)...')

        try:
            # --- Configurazione URDF e Pinocchio (Invariata) ---
            ...
            self.get_logger().info(f"Generating clean URDF using xacro: {' '.join(xacro_command)}")

            # 1. Esegui xacro
            with open(temp_urdf_file_raw, 'w') as f:
                subprocess.run(xacro_command, check=True, stdout=f, stderr=subprocess.PIPE)
            
            # 2. Sostituisci package:// con percorsi assoluti
            ...
            self.get_logger().error(f"Failed to load Pinocchio model or process URDF: {e}")
            # Non solleviamo, ma loggiamo l'errore per coerenza con l'originale, anche se l'inizializzazione fallirà.
            raise 

        # --- Inizializzazione Giunti (Invariata) ---
        ...
        # --- Publisher & Subscriber (Invariata) ---
    ...
    def joint_callback(self, msg):
        ...
        # Esegui la cinematica diretta (Invariata)
        pin.forwardKinematics(self.model, self.data, self.q)
        pin.updateFramePlacements(self.model, self.data)

        # Ottieni la posa dell'end-effector
        try:
            ...
            if ee_frame_id < self.model.nframes:
                ...
                # --- MODIFICA CHIAVE: Conversione Matrice di Rotazione in Quaternione ---
                # Pinocchio fornisce utility per la conversione. Il quaternione è (x, y, z, w).
                quaternion_w_xyz = pin.Quaternion(rot_matrix).coeffs() 
                
                # Pinocchio storage è (x, y, z, w), lo stesso di geometry_msgs/Quaternion
                quaternion = quaternion_w_xyz.tolist() # Converti in lista [x, y, z, w]
                # --- FINE MODIFICA CHIAVE ---

                # Pubblica la posa come PoseStamped
...
def main(args=None):
    ...
    # try/finally per garantire che la chiusura avvenga anche in caso di eccezioni in __init__
    node = None
    try:
        node = UR10eFKNode()
        rclpy.spin(node)
        ...
        if node:
             node.get_logger().error(f"Node initialization or runtime failed: {e}")
        else:
             print(f"Node initialization failed before logger was set up: {e}")
    finally:
        if node:
            node.destroy_node()
        rclpy.shutdown()
